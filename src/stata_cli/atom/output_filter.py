#!/usr/bin/env python3
"""
Output filtering helpers for the local CLI backend.
...
def apply_compact_mode_filter(output: str, filter_command_echo: bool = False) -> str:
    ...
    - CLI execution header lines (">>> [timestamp] do 'filepath'")
...
def check_token_limit_and_save(
    ...
    try:
        ...
        if not logs_dir:
            candidate = os.path.join(tempfile.gettempdir(), 'stata_cli_logs')
            tried_paths.append(candidate)
            try:
                os.makedirs(candidate, exist_ok=True)
                logs_dir = candidate
...
def process_output(
    ...
    for_cli_output: bool = True,
    filter_command_echo: bool = False
) -> str:
    """Process output for CLI returns, applying compact mode and token limits.
    ...
    Args:
        ...
        for_cli_output: Whether this is for CLI output (applies filters) or raw display
        filter_command_echo: Whether to filter command echo lines

    Returns:
        Processed output string
    """
    if not for_cli_output:
        # For direct display, return full output
        return output
