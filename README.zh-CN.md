# 面向 Codex 与原生 MCP 客户端的 Stata MCP
...
这个仓库现在的主形态是 **独立原生 MCP 服务器**，优先面向 [OpenAI Codex](https://github.com/openai/codex) 等 MCP 客户端，通过 **stdio** 直接拉起，不再依赖 VS Code、Cursor、Antigravity 或 `mcp-proxy`。

旧的 VS Code/Cursor/Antigravity 扩展资产仍然保留为 legacy 兼容路径，但它们已经不是推荐安装方式，也不是主产品形态。

## 快速开始

1. 准备 Python 3.11 环境。`stata-mcp` 目前固定为 `>=3.11,<3.12`，因为 Stata 的 Python 桥接在更新的 3.14 运行时下不兼容。
2. 安装依赖：
...
4. 或者直接接入 Codex，编辑 `~/.codex/config.toml`：
...
现在推荐直接把独立原生 MCP 服务接入 [OpenAI Codex](https://github.com/openai/codex)，不再需要 `mcp-proxy`。

1. 先用 Python 3.11 准备项目环境：
   ```bash
   uv sync --all-extras --python 3.11
   ```

2. 编辑 `~/.codex/config.toml` 来配置 Codex：
...
3. 如果文件里已经有其他 MCP 服务器，只需要加入 `[mcp_servers.stata-mcp]` 这一段。

4. 重启 Codex 或您的 IDE。

5. Codex 会自动发现这些原生 Stata 工具：
   - `stata_run_selection`
   - `stata_run_file`
   - `stata_list_sessions`
...
1. 确认项目虚拟环境是 Python 3.11：`./.venv/bin/python --version`
2. 检查 `~/.codex/config.toml` 是否指向 `uv run --directory ... stata-mcp`
3. 手动测试启动：
   ```bash
   uv run stata-mcp --stata-path /Applications/Stata
   ```
4. 检查本地日志文件（默认是临时目录下的 `stata_mcp_stdio.log`）
5. 如需排查旧的 HTTP 路径，可单独运行 `uv run stata-mcp-http ...`，但 Codex 仍应使用 stdio 入口
